...
export class PropertyPage {
  ...
  private init(){
    ...
    loading.present();
    this.merchant.getPropertyList()
    .map(res=>res.json())
    .subscribe(
      data=>{
        this.propertyList = data;
        this.tmp_propertyList = data;
        if(data){
  ...
  search(ev){
    ...
    if (val && val.trim() != '') {
      ...
      this.merchant.searchProperty(val)
      .map(res=>res.json())
      .subscribe(
        data=>{
      ...
      this.tmp_propertyList = this.propertyList.filter((item) => {
        return (item.NAME.toLowerCase().indexOf(val.toLowerCase()) > -1);
      });
    }
    else{
      this.tmp_propertyList = this.propertyList;
    }
  }
  ...
  public openDetail(id, img1, img2, img3, product, title){
    ...
    let detailData:any;
    loading.present();
    this.merchant.getDetailProperty({id: id}, {})
    .map(res=>res.text())
    .subscribe(
