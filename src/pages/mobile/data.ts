...
export class DataPage{
  ...
  inputHandle(input:any){
    ...
    if(code.length >= 4){
      ...
      this.mobile.getPacketOperator(code)
      ...
      .subscribe(
        ...
        ()=>{
          this.mobile.getPacketListOperator(this.op_code)
          .map(res=>res.json())
          .subscribe(
            data=>{
              this.op_packetList = data;
            },
            onerror=>{this.presentToast("Failed to get operator data, try re-enter number", 3500, 'bottom')},
            ()=>{}
          );
    ...
    .subscribe(
      data=>{
        ...
        if(parseInt(data) < this.op_price){
          ...
          loading.present();
          this.mobile.topUpPacket(this.phone, this.op_code, this.op_price, window.localStorage.getItem('id_user'))
          .map(res=>res.text())
          .subscribe(
            data=>{ 
              console.log(data);
