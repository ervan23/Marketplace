...
import {FashionService} from '../../providers/fashion-service';
...
export class FashionPage {
  ...
  private init(){
    ...
    loading.present();
    this.fashion.getFashionList()
    .map(res=>res.json())
    .subscribe(
      data=>{
        this.fashionList = data;
        this.tmp_fashionList = data;
        if(data){
  ...
  search(ev){
    ...
    if (val && val.trim() != '') {
      ...
      this.fashion.searchProduct(val)
      .map(res=>res.json())
      .subscribe(
        data=>{
      ...
      this.tmp_fashionList = this.fashionList.filter((item) => {
        return (item.NAME.toLowerCase().indexOf(val.toLowerCase()) > -1);
      });
    }
    else{
      this.tmp_fashionList = this.fashionList;
    }
  }
  ...
  public openDetail(id, img1, img2, img3, product, title){
    ...
    let detailData:any;
    loading.present();
    this.fashion.getDetailProduct({id: id}, {})
    .map(res=>res.text())
    .subscribe(
