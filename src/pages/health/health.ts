...
export class HealthPage {
  ...
  private init(){
    ...
    loading.present();
    this.health.getTravelList()
    .map(res=>res.json())
    .subscribe(
      data=>{
        this.healthList = data;
        this.tmp_healthList = data;
        if(data){
  ...
  search(ev){
    ...
    if (val && val.trim() != '') {
      ...
      this.tmp_healthList = this.healthList.filter((item) => {
        return (item.NAME.toLowerCase().indexOf(val.toLowerCase()) > -1);
      });
    }
    else{
      this.tmp_healthList = this.healthList;
    }
  }
  ...
  public openDetail(id, img1, img2, img3, product, title){
    ...
    let detailData:any;
    loading.present();
    this.health.getDetailProduct({id: id}, {})
    .map(res=>res.text())
    .subscribe(
