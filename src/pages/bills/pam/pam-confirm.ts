...
import {PamSummary} from './pam-summary';


@Component({
  selector: 'page-pln',
  templateUrl: 'pam-confirm.html',
  providers: [BillingService]
})
export class PamConfirm {
          ...
          data=>{
              if(parseInt(data) < this.total_amount){
                this.showAlert("Info", "You dont have enough balance, please topup first");
                  ...
                  loading.present();
                  this.billing.payPam(this.no_invoice, window.localStorage.getItem('id_user'), this.id_customer, this.total_amount)
                  .map(res=>res.text().split(';'))
                  .subscribe(
                      data=>{
                          if(data[0] == "$9"){
                            this.ev.publish("balance:reduce", this.total_amount);
                            this.navCtrl.push(PamSummary, {customer_data: data, admin: this.biaya_admin});
                          }
                          else{
