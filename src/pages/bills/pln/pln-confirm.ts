...
import {PlnSummary} from './pln-summary';


@Component({
  selector: 'page-pln',
  templateUrl: 'pln-confirm.html',
  providers: [BillingService]
})
export class PlnConfirm {
          ...
          data=>{
              if(parseInt(data) < this.total_amount){
                this.showAlert("Info", "You dont have enough balance, please topup first");
                  ...
                  loading.present();
                  this.billing.payPLN(this.no_invoice, window.localStorage.getItem('id_user'), this.id_customer, this.total_amount)
                  .map(res=>res.text().split(';'))
                  .subscribe(
                      data=>{
                          if(data[0] == "$1"){
                            this.ev.publish("balance:reduce", this.total_amount);
                            this.navCtrl.push(PlnSummary, {customer_data: data, admin:this.biaya_admin});
                          }
                          else{
