...
import {BpjsSummary} from './bpjs-summary';


@Component({
  selector: 'page-pln',
  templateUrl: 'bpjs-confirm.html',
  providers: [BillingService]
})
export class BpjsConfirm {
          ...
          data=>{
              if(parseInt(data) < this.total_amount){
                this.showAlert("Info", "You dont have enough balance, please topup first");
                  ...
                  loading.present();
                  this.billing.payBpjs(this.no_invoice, window.localStorage.getItem('id_user'), this.id_customer, this.total_amount)
                  .map(res=>res.text().split(';'))
                  .subscribe(
                      data=>{
                          if(data[0] == "$7"){
                            this.ev.publish("balance:reduce", this.total_amount);
                            this.navCtrl.push(BpjsSummary, {customer_data: data, admin: this.biaya_admin});
                          }
                          else{
