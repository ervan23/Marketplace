...
export class OthersPage {
  ...
  private init(){
    ...
    loading.present();
    this.other.getOtherList()
    .map(res=>res.json())
    .subscribe(
      data=>{
        this.otherList = data;
        this.tmp_otherList = data;
        if(data){
  ...
  search(ev){
    ...
    if (val && val.trim() != '') {
      ...
      this.tmp_otherList = this.otherList.filter((item) => {
        return (item.NAME.toLowerCase().indexOf(val.toLowerCase()) > -1);
      });
    }
    else{
      this.tmp_otherList = this.otherList;
    }
  }
  ...
  public openDetail(id, img1, img2, img3, product, title){
    ...
    let detailData:any;
    loading.present();
    this.other.getDetailProduct({id: id}, {})
    .map(res=>res.text())
    .subscribe(
