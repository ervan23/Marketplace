...
import {ReturnFlight} from './return-flight';


@Component({
  selector: 'page-plane',
  templateUrl: 'result-plane.html',
  providers: [PlaneService]
})
export class ResultPlane {
  ...
  {
    this.search_result = this.navParams.get('result');
    this.isReturn = this.navParams.get('return');
    this.retrun_flight = this.navParams.get('return_flight');
  }
  ...
  showSchedule(id_flight){
    ...
    if(this.isReturn){
      loading.dismiss();
      this.navCtrl.push(ReturnFlight, {departure_id: id_flight, retrun_flight: this.retrun_flight});
    }
    else{
      ...
      .subscribe(
          data=>{
              loading.dismiss();
              this.navCtrl.push(PlaneSchedule, {schedule: data[0]});
          },
          onerror=>{
              loading.dismiss();
